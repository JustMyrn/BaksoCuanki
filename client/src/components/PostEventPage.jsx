...
function PostEventPage({ onBack, onNext, readOnly }) {
  ...
  const [fotoCheckoutList, setFotoCheckoutList] = useState(d.fotoCheckoutHotel || []);
  
  const [jenisTransportasi, setJenisTransportasi] = useState(d.jenisTransportasi || '');
  const [detailTransportasi, setDetailTransportasi] = useState(d.detailTransportasi || '');
  const [gunakanKapalLaut, setGunakanKapalLaut] = useState(d.gunakanKapalLaut ?? null);

  const [jenisOpen, setJenisOpen] = useState(false);
  const [detailOpen, setDetailOpen] = useState(false);

  const [tiketList, setTiketList] = useState(d.tiketTransportasi || []);
  const [nominalBiayaTiket, setNominalBiayaTiket] = useState(d.nominalBiayaTiket || '');

  const [uangHarian, setUangHarian] = useState(d.uangHarianTambahan || false);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(draftKey, JSON.stringify({
        fotoCheckoutHotel: fotoCheckoutList,
        jenisTransportasi,
        detailTransportasi,
        gunakanKapalLaut,
        tiketTransportasi: tiketList,
        nominalBiayaTiket,
  ...
  }, [fotoCheckoutList, jenisTransportasi, detailTransportasi, gunakanKapalLaut, tiketList, nominalBiayaTiket, uangHarian, ketUangHarian, transportTambahan, ketTransportTambahan, notaList, nominalTambahan]);
  ...
  const handleAmbilGambar = async () => {
    ...
    if (foto) {
      if (cameraTarget === 'checkout') setFotoCheckoutList(p => [...p, foto]);
      else if (cameraTarget === 'tiket') setTiketList(p => [...p, foto]);
      else if (cameraTarget === 'nota') setNotaList(p => [...p, foto]);
    }
  ...
  const hapusFotoCheckout = (i) => setFotoCheckoutList(p => p.filter((_, idx) => idx !== i));
  const hapusTiket = (i) => setTiketList(p => p.filter((_, idx) => idx !== i));

  const handleJenisChange = (v) => { setJenisTransportasi(v); setDetailTransportasi(''); setGunakanKapalLaut(null); setJenisOpen(false); };
  ...
  const detailOptions = DETAIL_TRANSPORTASI[jenisTransportasi] || [];
  ...
  return (
    <div className="relative mx-auto flex min-h-dvh w-full max-w-[390px] flex-col bg-white font-['Inter'] sm:max-w-md">
      ...
      <div className={`mt-[20px] flex flex-col gap-[20px] px-[25px] ${readOnly ? 'pointer-events-none opacity-60 select-none' : ''}`}>
        {/* Foto Check Out Hotel */}
        <div className="flex flex-col gap-1">
          <span className="text-[13px] font-bold text-black">Foto Check Out Hotel</span>
          <button onClick={() => openCameraFor('checkout')} className="flex h-[49px] w-full flex-col items-center justify-center gap-1 rounded-[7px] bg-[#D9D9D9] text-black hover:bg-[#C0C0C0] transition-colors">
          ...
          {fotoCheckoutList.map((f,i) => (
            <div key={i} className="relative rounded-[7px] border border-[#D9D9D9] overflow-hidden mt-1">
              <img src={f.dataUrl} alt={`Foto Checkout ${i+1}`} className="h-auto w-full object-contain" />
              ...
              <button onClick={() => hapusFotoCheckout(i)} className="absolute right-1 top-1 flex h-[22px] w-[22px] items-center justify-center rounded-full bg-red-500 text-[12px] font-bold text-white">✕</button>
            </div>
          ))}
        </div>
        ...
        {/* Alat Transportasi */}
        <div className="flex flex-col gap-1">
          <label className="text-[13px] font-bold text-black">Alat Transportasi yang dipergunakan untuk kembali</label>
        ...
        {/* Cuma muncul kalau sudah pilih detail transportasi */}
        {showUploadTiket && (
          <>
            {/* Upload Tiket & Nominal */}
            <div className="flex flex-col gap-1">
              <span className="text-[13px] font-bold text-black">Upload Tiket Transportasi Pulang</span>
              <button onClick={() => openCameraFor('tiket')} className="flex h-[49px] w-full flex-col items-center justify-center gap-1 rounded-[7px] bg-[#D9D9D9] text-black hover:bg-[#C0C0C0] transition-colors">
                <svg width="13" height="11" viewBox="0 0 18 14" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M16 3H13.5L12 1H6L4.5 3H2C1.2 3 0.5 3.7 0.5 4.5V12.5C0.5 13.3 1.2 14 2 14H16C16.8 14 17.5 13.3 17.5 12.5V4.5C17.5 3.7 16.8 3 16 3Z"/><circle cx="9" cy="8" r="3"/></svg>
                <span className="text-[7px] font-semibold leading-none">Ambil Gambar</span>
              </button>
              {tiketList.map((f,i) => (
                <div key={i} className="relative rounded-[7px] border border-[#D9D9D9] overflow-hidden mt-1">
                  <img src={f.dataUrl} alt={`Tiket ${i+1}`} className="h-auto w-full object-contain" />
                  <div className="absolute bottom-0 left-0 right-0 bg-black/60 px-2 py-1 text-[10px] text-white">{f.timestamp}{f.lat && ` — GPS: ${f.lat.toFixed(4)},${f.lng.toFixed(4)}`}</div>
                  <button onClick={() => hapusTiket(i)} className="absolute right-1 top-1 flex h-[22px] w-[22px] items-center justify-center rounded-full bg-red-500 text-[12px] font-bold text-white">✕</button>
                </div>
              ))}
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[13px] font-bold text-black">Masukan Nominal Biaya</label>
              <input type="text" inputMode="numeric" value={nominalBiayaTiket} onChange={(e) => setNominalBiayaTiket(e.target.value)} className="h-[27px] w-full rounded-[7px] bg-[#D9D9D9] px-2 text-[13px] font-bold text-black outline-none" />
            </div>
          </>
        )}
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <div onClick={() => setUangHarian(!uangHarian)} className={`flex h-4 w-4 cursor-pointer items-center justify-center rounded-[2px] border-2 border-[#E5E6EB] ${uangHarian ? 'bg-blue-500 border-blue-500' : 'bg-white'}`}>
                      {uangHarian && <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 5l2 2 4-4" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                    <span className="text-[13px] font-bold text-[#1D2129]">Uang Harian Tambahan</span>
                  </div>
                  <div className="pl-6 w-full">
                    <input type="text" value={ketUangHarian} onChange={e => setKetUangHarian(e.target.value)} placeholder="Parkir, TopUp E-Money, Uang Makan, dan lainnya." className="h-[30px] w-full rounded-[7px] bg-[#D9D9D9] px-2 text-[12px] placeholder:italic placeholder:font-[400] placeholder:text-gray-600 outline-none" />
                  </div>
                </div>

                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <div onClick={() => setTransportTambahan(!transportTambahan)} className={`flex h-4 w-4 cursor-pointer items-center justify-center rounded-[2px] border-2 border-[#E5E6EB] ${transportTambahan ? 'bg-blue-500 border-blue-500' : 'bg-white'}`}>
                      {transportTambahan && <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 5l2 2 4-4" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                    <span className="text-[13px] font-bold text-[#1D2129]">Transport Tambahan</span>
                  </div>
                  <div className="pl-6 w-full">
                    <input type="text" value={ketTransportTambahan} onChange={e => setKetTransportTambahan(e.target.value)} placeholder="Ongkos Transum, Ojek, Bus, Kereta, dan lainnya." className="h-[30px] w-full rounded-[7px] bg-[#D9D9D9] px-2 text-[12px] placeholder:italic placeholder:font-[400] placeholder:text-gray-600 outline-none" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="mt-auto flex items-center justify-between px-[25px] pb-[40px] pt-[20px]">
        <button onClick={onBack} className="flex h-[24px] min-w-[67px] items-center justify-center rounded-[35px] bg-[#04305F] px-2 text-white hover:brightness-110 active:scale-95">
          <span className="text-[14px] font-[800]">Back</span>
        </button>
        <button onClick={() => {
          if (readOnly) {
            onNext?.(); return;
          ...
          if (showUploadTiket) {
            if (tiketList.length === 0) missing.push('Upload Tiket Transportasi');
            if (!nominalBiayaTiket.trim()) missing.push('Nominal Biaya Tiket');
          }
          ...
          const draftData = {
            fotoCheckoutHotel: fotoCheckoutList,
            jenisTransportasi, detailTransportasi, gunakanKapalLaut: gunakanKapalLaut ?? false,
            tiketTransportasi: tiketList, nominalBiayaTiket,
            uangHarianTambahan: uangHarian, ketUangHarian,
            transportTambahan, ketTransportTambahan,
            notaBiayaTambahan: notaList, nominalBiayaTambahan: nominalTambahan
          };
