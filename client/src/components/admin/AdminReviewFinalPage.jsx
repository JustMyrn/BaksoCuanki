...
export default function AdminReviewFinalPage({ onBack, onLogout, onNavigate, reviewData }) {
  ...
  const handleApprove = async () => {
    try {
      ...
      // await fetch(`${API}/api/admin/perjalanan-dinas/${emp.perjalananId}/approve`, { method: 'POST', headers: { Authorization: `Bearer ${t}` } });
      // alert('Perjalanan disetujui (Fitur API belum tersambung sepenuhnya).');
      onNavigate?.('review-approved');
    } catch (e) {
      console.error(e);
  ...
  const handleReject = async () => {
    try {
      const t = localStorage.getItem('integra_admin_token');
      // Replace with actual API call later
      // await fetch(`${API}/api/admin/perjalanan-dinas/${emp.perjalananId}/reject`, { method: 'POST', headers: { Authorization: `Bearer ${t}` } });
      // alert('Perjalanan ditolak (Fitur API belum tersambung sepenuhnya).');
      onNavigate?.('review-denied');
    } catch (e) {
      console.error(e);
    ...
    <div className="flex h-screen w-full overflow-hidden font-['Inter']">
      ...
      <main className="flex flex-1 flex-col bg-[#072D6C]">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-10">
          <h2 className="text-[27px] font-extrabold text-white text-center">Apakah Perjalanan Dinas Ini Disetujui?</h2>
          <div className="flex items-center gap-10 mt-2">
            <button
              onClick={handleApprove}
              className="w-[159px] h-12 bg-white rounded-[10px] text-black text-2xl font-extrabold hover:bg-gray-200 transition"
            >
              Ya
            </button>
            <button
